namespace eval ::optrace {
  variable script "/home/nmath018/topK_feature/hls_catch22/_x.hw.xilinx_u250_gen3x16_xdma_4_1_202210_1/link/vivado/vpl/vpl.tcl"
...
OPTRACE "Collect BD interface connectivity and write automation summary report" END { }
  ...
  set bd_file [ocl_util::get_bd_file $hw_platform_info $config_info]
  set top_bd_file [dict get $hw_platform_info hw_platform_top_bd]
  set top_bd_file [file tail $top_bd_file]
  if {![ocl_util::is_empty $top_bd_file] && $top_bd_file != $bd_file && [get_files -quiet $top_bd_file] != ""} {
    set top_bd_name [file rootname $top_bd_file]
    set cur_bd [current_bd_design]
    ocl_util::add_to_steps_log $steps_log "internal step: open_bd_design -auto_upgrade \[get_files $top_bd_file\]" [ocl_util::extFileName]:[expr {[dict get [info frame -1] line] + [dict get [info frame 0] line]}]
    open_bd_design -auto_upgrade [get_files -all $top_bd_file]
    ocl_util::add_to_steps_log $steps_log "internal step: save_bd_design" [ocl_util::extFileName]:[expr {[dict get [info frame -1] line] + [dict get [info frame 0] line]}]
    save_bd_design
    ocl_util::add_to_steps_log $steps_log "internal step: close_bd_design $top_bd_name" [ocl_util::extFileName]:[expr {[dict get [info frame -1] line] + [dict get [info frame 0] line]}]
    close_bd_design $top_bd_name
...
OPTRACE "Generate output products" START { }
  ...
  set top_bd_file [file tail $top_bd_file]
  if {![ocl_util::is_empty $top_bd_file] && $top_bd_file != $bd_file && [get_files -quiet $top_bd_file] != ""} {
    set target_bd_file $top_bd_file
  } else {
    set target_bd_file $bd_file
  }
  ocl_util::add_to_steps_log $steps_log "internal step: generate_target all \[get_files $target_bd_file\]" [ocl_util::extFileName]:[expr {[dict get [info frame -1] line] + [dict get [info frame 0] line]}]
  generate_target all [get_files $target_bd_file]
...
set rc [catch {
  ...
  if {$enable_multi_strategies} {
    ...
    foreach strategy $run_strategies {
      ...
      if { [get_runs $st_run_name] ne "" } {
        set _run [get_runs $st_run_name]
        puts "INFO: \[OCL_UTIL\] impl run $_run already exists, no need to re-create"
      } else {
        if {[string equal $hw_platform_state "impl"]} {
...
set rc [catch {
  ...
  set prev_val [ocl_util::set_max_threads_param_synth]
...
OPTRACE "Synthesis" END { }
  file delete __vpl_running_synthesis__
  ocl_util::restore_max_threads_param $prev_val
